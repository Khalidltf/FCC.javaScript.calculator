...
function App() {
  ...
  const calculate = () => {
    if (isOperator(ext.charAt(ext.length - 1))) return;
    const result = ext.split(" ");
    const newResult = [];

    for (let i = result.length - 1; i >= 0; i--) {
      if (["*", "/", "+"].includes(result[i]) && isOperator(result[i - 1])) {
        newResult.unshift(result[i]);
        let j = 0;
        let k = i - 1;
        while (isOperator(result[k])) {
          k--;
          j++;
        }
        i -= j;
    ...
    const newExpression = newResult.join(" ");
